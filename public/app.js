...
const PERMISSIONS = {
  admin: {
    pages:        ['dashboard','products','withdraw','receive','transactions','history','analytics','userLog','userManagement','batchScan','shiftReport','purchaseOrder'],
  ...
  warehouse: {
    pages:        ['dashboard','products','withdraw','receive','transactions','history','analytics','userLog','batchScan','shiftReport','purchaseOrder'],
...
function refreshCurrentView() {
  const renderers = {
    ...
    batchScan: renderBatch, shiftReport: renderShiftReport, purchaseOrder: renderPO,
...
function toggleFullscreen() {
  ...
  if (isShowing) {
    ...
    kpiClockInterval = setInterval(() => {
      ...
      if (clk) { const n = new Date(); clk.textContent = `${String(n.getHours()).padStart(2,'0')}:${String(n.getMinutes()).padStart(2,'0')}:${String(n.getSeconds()).padStart(2,'0')}`; }
      renderKpiFullscreen();
    }, 5000);
  }
}
...
function renderKpiFullscreen() {
  ...
  grid.innerHTML = `
    <div class="kpi-fs-card kpi-out"><div class="kpi-fs-icon"><i class="fas fa-arrow-right-from-bracket"></i></div><div class="kpi-fs-val">${todayOut.toLocaleString()}</div><div class="kpi-fs-lbl">เบิกออกวันนี้</div></div>
    <div class="kpi-fs-card kpi-in"><div class="kpi-fs-icon"><i class="fas fa-arrow-right-to-bracket"></i></div><div class="kpi-fs-val">${todayIn.toLocaleString()}</div><div class="kpi-fs-lbl">รับเข้าวันนี้</div></div>
    <div class="kpi-fs-card kpi-stock"><div class="kpi-fs-icon"><i class="fas fa-boxes-stacked"></i></div><div class="kpi-fs-val">${totalStock.toLocaleString()}</div><div class="kpi-fs-lbl">สต็อกรวม</div></div>
    <div class="kpi-fs-card ${lowCount>0?'kpi-alert':'kpi-ok'}"><div class="kpi-fs-icon"><i class="fas fa-triangle-exclamation"></i></div><div class="kpi-fs-val">${lowCount}</div><div class="kpi-fs-lbl">สินค้าใกล้หมด</div></div>
    <div class="kpi-fs-card kpi-tx"><div class="kpi-fs-icon"><i class="fas fa-receipt"></i></div><div class="kpi-fs-val">${totalTx}</div><div class="kpi-fs-lbl">รายการวันนี้</div></div>
    <div class="kpi-fs-card kpi-last"><div class="kpi-fs-icon"><i class="fas fa-satellite-dish"></i></div><div class="kpi-fs-val" style="font-size:18px;">${lastTx ? escapeHtml(lastTx.product||lastTx.rfid||'-') : '—'}</div><div class="kpi-fs-lbl">สแกนล่าสุด</div></div>`;

  if (ticker) {
    const recentNames = transactions.slice(0, 10).map(t => `${t.type==='out'?'🔴 เบิก':'🟢 รับ'} ${t.product||t.rfid||'-'} × ${t.qty}`).join('　　　');
    ticker.textContent = recentNames || 'รอข้อมูล...';
  }
}
...
let poList = [];
let poFilter = 'all';
let poItems = [];   // items in modal
let currentPOId = null; // for detail/print

const PO_STATUS = {
  pending:   { label: 'รอดำเนินการ', color: '#F59E0B', bg: 'rgba(245,158,11,.1)',  icon: 'fa-clock' },
  approved:  { label: 'อนุมัติแล้ว',  color: '#6366f1', bg: 'rgba(99,102,241,.1)', icon: 'fa-check-circle' },
  ordered:   { label: 'สั่งซื้อแล้ว', color: '#0EA5E9', bg: 'rgba(14,165,233,.1)', icon: 'fa-truck' },
  received:  { label: 'รับของแล้ว',  color: '#22C55E', bg: 'rgba(34,197,94,.1)',  icon: 'fa-box-check' },
  cancelled: { label: 'ยกเลิก',      color: '#EF4444', bg: 'rgba(239,68,68,.1)',  icon: 'fa-ban' },
};

function getPOStatusBadge(status) {
  const s = PO_STATUS[status] || { label: status, color: '#888', bg: '#f0f0f0', icon: 'fa-circle' };
  return `<span class="po-status-badge" style="background:${s.bg};color:${s.color};"><i class="fas ${s.icon}"></i> ${s.label}</span>`;
}
...
function setPOFilter(f, el) {
  poFilter = f;
  document.querySelectorAll('#purchaseOrderView .filter-chip').forEach(c => c.classList.remove('active'));
  if (el) el.classList.add('active');
  renderPO();
}
...
function renderPOSummary() {
  const el = document.getElementById('poSummaryRow');
  if (!el) return;
  const counts = { pending: 0, approved: 0, ordered: 0, received: 0, cancelled: 0 };
  let totalValue = 0;
  poList.forEach(p => {
    counts[p.status] = (counts[p.status] || 0) + 1;
    totalValue += calcPOTotal(p.items);
  });
  el.innerHTML = `
    <div class="po-sum-card"><div class="po-sum-val" style="color:#F59E0B;">${counts.pending}</div><div class="po-sum-lbl">รอดำเนินการ</div></div>
    <div class="po-sum-card"><div class="po-sum-val" style="color:#6366f1;">${counts.approved}</div><div class="po-sum-lbl">อนุมัติแล้ว</div></div>
    <div class="po-sum-card"><div class="po-sum-val" style="color:#0EA5E9;">${counts.ordered}</div><div class="po-sum-lbl">สั่งซื้อแล้ว</div></div>
    <div class="po-sum-card"><div class="po-sum-val" style="color:#22C55E;">${counts.received}</div><div class="po-sum-lbl">รับของแล้ว</div></div>
    <div class="po-sum-card po-sum-total"><div class="po-sum-val">฿${totalValue.toLocaleString('th-TH', {minimumFractionDigits:0})}</div><div class="po-sum-lbl">มูลค่ารวมทั้งหมด</div></div>`;
}

function renderPOTable() {
  const b = document.getElementById('poTableBody');
  if (!b) return;
  const filtered = poFilter === 'all' ? poList : poList.filter(p => p.status === poFilter);
  if (!filtered.length) {
    b.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:40px;color:var(--text-muted);"><i class="fas fa-file-invoice" style="font-size:28px;display:block;margin-bottom:8px;opacity:.3;"></i>ยังไม่มีใบสั่งซื้อ</td></tr>`;
    return;
  }
  const canManage = can('users', 'view'); // admin/warehouse only
  b.innerHTML = filtered.map(po => {
    const total = calcPOTotal(po.items);
    const itemCount = (po.items || []).length;
    return `<tr>
      <td><strong style="font-family:monospace;color:var(--accent);">${po.poNumber}</strong></td>
      <td class="product-name">${escapeHtml(po.supplier)}</td>
      <td>${itemCount} รายการ</td>
      <td><strong>฿${total.toLocaleString('th-TH', {minimumFractionDigits:0})}</strong></td>
      <td>${getPOStatusBadge(po.status)}</td>
      <td style="font-size:12px;color:var(--text-muted);">${fmtTime(po.createdAt)}</td>
      <td>${escapeHtml(po.createdBy || '-')}</td>
      <td>
        <div class="action-btns">
          <button class="action-btn edit" onclick="viewPODetail(${po.id})" title="ดูรายละเอียด"><i class="fas fa-eye"></i></button>
          ${canManage && po.status === 'pending' ? `<button class="action-btn" style="color:#6366f1;" onclick="updatePOStatus(${po.id},'approved')" title="อนุมัติ"><i class="fas fa-check"></i></button>` : ''}
          ${canManage && po.status === 'approved' ? `<button class="action-btn" style="color:#0EA5E9;" onclick="updatePOStatus(${po.id},'ordered')" title="สั่งซื้อแล้ว"><i class="fas fa-truck"></i></button>` : ''}
          ${canManage && po.status === 'ordered' ? `<button class="action-btn" style="color:#22C55E;" onclick="receivePO(${po.id})" title="รับของแล้ว"><i class="fas fa-box-check"></i></button>` : ''}
          ${canManage && po.status === 'pending' ? `<button class="action-btn delete" onclick="cancelPO(${po.id})" title="ยกเลิก"><i class="fas fa-ban"></i></button>` : ''}
        </div>
      </td>
    </tr>`;
  }).join('');
...
function openPOModal(id) {
  ...
  document.getElementById('poModalTitle').innerHTML = '<i class="fas fa-file-invoice text-cyan"></i> สร้างใบสั่งซื้อใหม่';
  document.getElementById('poSupplier').value = '';
  document.getElementById('poNote').value = '';
  poItems = [];
  // Auto-suggest: add low-stock products
  products.filter(p => p.minStock > 0 && p.quantity <= p.minStock).slice(0, 3).forEach(p => {
    poItems.push({ name: p.name, qty: Math.max(p.minStock - p.quantity + p.minStock, 1), unit: p.unit, price: 0 });
  });
  if (!poItems.length) poItems.push({ name: '', qty: 1, unit: 'ชิ้น', price: 0 });
  renderPOItems();
  openModal('poModal');
}

function addPOItem() {
  poItems.push({ name: '', qty: 1, unit: 'ชิ้น', price: 0 });
  renderPOItems();
}
...
function renderPOItems() {
  const b = document.getElementById('poItemsBody');
  if (!b) return;
  // Build product datalist
  const prodOptions = products.map(p => `<option value="${escapeHtml(p.name)}" data-unit="${p.unit}">`).join('');
  b.innerHTML = poItems.map((item, i) => `
    <tr>
      <td><input list="poProductList" value="${escapeHtml(item.name)}" placeholder="ชื่อสินค้า" style="width:100%;padding:5px 8px;border:1px solid var(--border);border-radius:6px;font-size:12px;"
        oninput="updatePOItem(${i},'name',this.value);autofillPOUnit(${i},this.value)"></td>
      <td><input type="number" min="1" value="${item.qty}" style="width:70px;padding:5px 8px;border:1px solid var(--border);border-radius:6px;text-align:center;font-size:12px;"
        oninput="updatePOItem(${i},'qty',this.value)"></td>
      <td><input value="${escapeHtml(item.unit)}" id="poUnit${i}" placeholder="ชิ้น" style="width:60px;padding:5px 8px;border:1px solid var(--border);border-radius:6px;font-size:12px;"
        oninput="updatePOItem(${i},'unit',this.value)"></td>
      <td><input type="number" min="0" step="0.01" value="${item.price}" placeholder="0.00" style="width:90px;padding:5px 8px;border:1px solid var(--border);border-radius:6px;text-align:right;font-size:12px;"
        oninput="updatePOItem(${i},'price',this.value)"></td>
      <td style="text-align:right;font-weight:700;font-size:12px;">฿${((parseFloat(item.price)||0)*(parseInt(item.qty)||0)).toLocaleString()}</td>
      <td><button class="action-btn delete" onclick="removePOItem(${i})"><i class="fas fa-trash"></i></button></td>
    </tr>`).join('');
  if (!document.getElementById('poProductList')) {
    const dl = document.createElement('datalist');
    dl.id = 'poProductList';
    dl.innerHTML = prodOptions;
    document.body.appendChild(dl);
  } else {
    document.getElementById('poProductList').innerHTML = prodOptions;
  }
  updatePOTotal();
}

function updatePOItem(idx, field, val) {
  if (poItems[idx]) poItems[idx][field] = field === 'qty' ? parseInt(val)||1 : field === 'price' ? parseFloat(val)||0 : val;
  updatePOTotal();
}

function autofillPOUnit(idx, name) {
  const p = products.find(x => x.name === name);
  if (p) {
    poItems[idx].unit = p.unit;
    const unitEl = document.getElementById(`poUnit${idx}`);
    if (unitEl) unitEl.value = p.unit;
  }
}

function updatePOTotal() {
  const total = calcPOTotal(poItems);
  const el = document.getElementById('poGrandTotal');
  if (el) el.textContent = `฿${total.toLocaleString('th-TH', {minimumFractionDigits:0})}`;
}

async function savePO() {
  const supplier = document.getElementById('poSupplier').value.trim();
  const note = document.getElementById('poNote').value.trim();
  if (!supplier) { showToast('error', 'กรุณาระบุซัพพลายเออร์'); return; }
  const validItems = poItems.filter(i => i.name.trim());
  if (!validItems.length) { showToast('error', 'กรุณาเพิ่มรายการสินค้าอย่างน้อย 1 รายการ'); return; }
  try {
    const data = { supplier, note, items: validItems, createdBy: currentUser?.name || '' };
    const result = await api('POST', '/po', data);
    poList.unshift(result);
    closeModal('poModal');
    showToast('success', `สร้าง PO สำเร็จ: ${result.poNumber}`);
    addLog('สร้าง PO', `${result.poNumber} — ${supplier} (${validItems.length} รายการ)`);
    pushNotif('info', 'สร้าง PO ใหม่', `${result.poNumber} — ${supplier}`, 'fa-file-invoice');
    renderPO();
    updatePOBadge();
...
async function updatePOStatus(id, status) {
  const labels = { approved: 'อนุมัติ', ordered: 'สั่งซื้อแล้ว', cancelled: 'ยกเลิก' };
  ...
  try {
    ...
    addLog('อัปเดต PO', `#${id} → ${labels[status] || status}`);
    renderPO();
    updatePOBadge();
  } catch(e) { showToast('error', e.message); }
}
...
async function receivePO(id) {
  ...
  if (!po) return;
  if (!confirm(`ยืนยันรับสินค้าตาม PO ${po.poNumber}?\nระบบจะเพิ่มสต็อกตามรายการอัตโนมัติ`)) return;
  try {
    // Update stock for each item that matches a product
    const now = getNow();
    for (const item of (po.items || [])) {
      const p = products.find(x => x.name === item.name || x.sku === item.name);
      if (!p) continue;
      const newQty = p.quantity + (parseInt(item.qty) || 0);
      await api('PATCH', `/products/${p.id}/quantity`, { quantity: newQty, updatedAt: now });
      const txData = { type: 'in', rfid: p.rfid || '', product: p.name, sku: p.sku, qty: parseInt(item.qty)||0, user: currentUser?.name || 'ระบบ', time: now, reason: `รับของ PO: ${po.poNumber}` };
      const txResult = await api('POST', '/transactions', txData);
      p.quantity = newQty;
      transactions.unshift({ id: txResult.id, ...txData });
      addToLiveFeedTx('in', p.rfid || '', p.name, parseInt(item.qty)||0);
    }
    await api('PATCH', `/po/${id}/status`, { status: 'received' });
    const idx = poList.findIndex(p => p.id == id);
    if (idx >= 0) poList[idx].status = 'received';
    addLog('รับของ PO', `${po.poNumber} — ${(po.items||[]).length} รายการ`);
    pushNotif('success', 'รับของ PO แล้ว', `${po.poNumber} — อัปเดตสต็อกอัตโนมัติ`, 'fa-box-check');
    showToast('success', `รับของ PO ${po.poNumber} สำเร็จ อัปเดตสต็อกแล้ว`);
    syncAllViews();
    renderPO();
    updatePOBadge();
  } catch(e) { showToast('error', e.message); }
...
function viewPODetail(id) {
  ...
  document.getElementById('poDetailTitle').innerHTML = `<i class="fas fa-file-invoice text-cyan"></i> ${po.poNumber}`;
  document.getElementById('poDetailBody').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;margin-bottom:20px;">
      <div class="shift-kpi" style="text-align:left;padding:14px;">
        <div style="font-size:11px;color:var(--text-muted);margin-bottom:4px;">ซัพพลายเออร์</div>
        <div style="font-weight:700;font-size:15px;">${escapeHtml(po.supplier)}</div>
      </div>
      <div class="shift-kpi" style="text-align:left;padding:14px;">
        <div style="font-size:11px;color:var(--text-muted);margin-bottom:4px;">สถานะ</div>
        <div>${getPOStatusBadge(po.status)}</div>
      </div>
      <div class="shift-kpi" style="text-align:left;padding:14px;">
        <div style="font-size:11px;color:var(--text-muted);margin-bottom:4px;">มูลค่ารวม</div>
        <div style="font-weight:800;font-size:18px;color:var(--accent);">฿${total.toLocaleString('th-TH',{minimumFractionDigits:0})}</div>
      </div>
    </div>
    <div style="margin-bottom:16px;font-size:12px;color:var(--text-muted);">
      สร้างโดย: <strong>${escapeHtml(po.createdBy||'-')}</strong> | วันที่: <strong>${fmtTime(po.createdAt)}</strong>
      ${po.note ? ` | หมายเหตุ: <strong>${escapeHtml(po.note)}</strong>` : ''}
    </div>
    <table class="data-table">
      <thead><tr><th>#</th><th>สินค้า</th><th>จำนวน</th><th>หน่วย</th><th>ราคา/หน่วย</th><th>รวม</th></tr></thead>
      <tbody>${(po.items||[]).map((item,i) => `<tr>
        ...
        <td><strong>฿${((parseFloat(item.price)||0)*(parseInt(item.qty)||0)).toLocaleString()}</strong></td>
      </tr>`).join('')}</tbody>
    </table>
    <div style="text-align:right;margin-top:12px;padding:12px 16px;background:var(--surface);border-radius:8px;font-size:16px;font-weight:800;">
      มูลค่ารวมทั้งสิ้น: <span style="color:var(--accent);margin-left:12px;">฿${total.toLocaleString('th-TH',{minimumFractionDigits:0})}</span>
    </div>
...
function printCurrentPO() {
  const po = poList.find(p => p.id == currentPOId);
  if (!po) return;
  const total = calcPOTotal(po.items);
  const w = window.open('', '_blank');
  w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${po.poNumber}</title>
  <style>
    body{font-family:sans-serif;padding:32px;color:#111;font-size:13px;max-width:800px;margin:0 auto;}
    ...
    .info-box{border:1px solid #eee;border-radius:8px;padding:12px;}
    .info-label{font-size:10px;color:#888;margin-bottom:4px;}
    .info-val{font-weight:700;}
    @media print{body{padding:0;}}
  </style></head><body>
  <h1>ใบสั่งซื้อ — ${po.poNumber}</h1>
  <div class="sub">RFID Factory Inventory System | พิมพ์เมื่อ ${getNow()}</div>
  <div class="info-grid">
    <div class="info-box"><div class="info-label">ซัพพลายเออร์</div><div class="info-val">${escapeHtml(po.supplier)}</div></div>
    <div class="info-box"><div class="info-label">สถานะ</div><div class="info-val">${PO_STATUS[po.status]?.label||po.status}</div></div>
    <div class="info-box"><div class="info-label">ผู้สร้าง / วันที่</div><div class="info-val">${escapeHtml(po.createdBy||'-')} | ${fmtTime(po.createdAt)}</div></div>
  </div>
  <table><thead><tr><th>#</th><th>สินค้า</th><th>จำนวน</th><th>หน่วย</th><th>ราคา/หน่วย</th><th>รวม</th></tr></thead>
  <tbody>${(po.items||[]).map((item,i)=>`<tr><td>${i+1}</td><td>${escapeHtml(item.name||'-')}</td><td>${item.qty}</td><td>${escapeHtml(item.unit||'ชิ้น')}</td><td>฿${(parseFloat(item.price)||0).toLocaleString()}</td><td>฿${((parseFloat(item.price)||0)*(parseInt(item.qty)||0)).toLocaleString()}</td></tr>`).join('')}</tbody>
  </table>
  <div class="total">มูลค่ารวมทั้งสิ้น: ฿${total.toLocaleString('th-TH',{minimumFractionDigits:2})}</div>
  ${po.note ? `<p style="margin-top:16px;color:#666;">หมายเหตุ: ${escapeHtml(po.note)}</p>` : ''}
  </body></html>`);
  w.document.close();
  setTimeout(() => { w.focus(); w.print(); }, 400);
}
